import React from 'react'

import Icon from '../Icon'

const MaterialDoNotTouchSharpSvg = props => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    enable-background="new 0 0 24 24"
    height="24"
    viewBox="0 0 24 24"
    width="24"
    {...props}
  >
    <g>
      <rect fill="none" height="24" width="24" />
      <path d="M13,10.17l-2.5-2.5V1H13V10.17z M20,4h-2.5v7h-1V2H14v9.17l6,6V4z M9.5,3H7.01v1.18L9.5,6.67V3z M21.19,21.19L2.81,2.81 L1.39,4.22L7,9.83v4.3l-3.32-1.9L2,13.88L9.68,22h9.54l0.56,0.61L21.19,21.19z" />
    </g>
  </svg>
)

const MaterialDoNotTouchSharp = props => <Icon as={MaterialDoNotTouchSharpSvg} {...props} />

MaterialDoNotTouchSharp.defaultProps = { ...Icon.defaultProps }

MaterialDoNotTouchSharp.propTypes = { ...Icon.propTypes }

export default MaterialDoNotTouchSharp

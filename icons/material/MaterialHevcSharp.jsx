import React from 'react'

import Icon from '../Icon'

const MaterialHevcSharpSvg = props => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    enable-background="new 0 0 24 24"
    height="24"
    viewBox="0 0 24 24"
    width="24"
    {...props}
  >
    <g>
      <rect fill="none" height="24" width="24" />
    </g>
    <g>
      <g>
        <polygon points="5.5,11 4.5,11 4.5,9 3,9 3,15 4.5,15 4.5,12.5 5.5,12.5 5.5,15 7,15 7,9 5.5,9" />
        <polygon points="21,11 21,9 17,9 17,15 21,15 21,13 19.5,13 19.5,13.5 18.5,13.5 18.5,10.5 19.5,10.5 19.5,11" />
        <polygon points="14.25,13.5 13.5,9 12,9 13,15 15.5,15 16.5,9 15,9" />
        <polygon points="8,9 8,15 11.5,15 11.5,13.5 9.5,13.5 9.5,12.5 11.5,12.5 11.5,11 9.5,11 9.5,10.5 11.5,10.5 11.5,9" />
      </g>
    </g>
  </svg>
)

const MaterialHevcSharp = props => <Icon as={MaterialHevcSharpSvg} {...props} />

MaterialHevcSharp.defaultProps = { ...Icon.defaultProps }

MaterialHevcSharp.propTypes = { ...Icon.propTypes }

export default MaterialHevcSharp

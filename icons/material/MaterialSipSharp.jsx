import React from 'react'

import Icon from '../Icon'

const MaterialSipSharpSvg = props => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    enable-background="new 0 0 24 24"
    height="24"
    viewBox="0 0 24 24"
    width="24"
    {...props}
  >
    <g>
      <rect fill="none" height="24" width="24" />
    </g>
    <g>
      <g>
        <rect height="1" width="2" x="15.5" y="10.5" />
        <path d="M20,4H4C2.9,4,2,4.9,2,6v12c0,1.1,0.9,2,2,2h16c1.1,0,2-0.9,2-2V6C22,4.9,21.1,4,20,4z M10,10.5H6.5v0.75H10V15H5v-1.5 h3.5v-0.75H5V9h5V10.5z M13,15h-2V9h2V15z M19,9v4h-3.5v2H14V9H19z" />
      </g>
    </g>
  </svg>
)

const MaterialSipSharp = props => <Icon as={MaterialSipSharpSvg} {...props} />

MaterialSipSharp.defaultProps = { ...Icon.defaultProps }

MaterialSipSharp.propTypes = { ...Icon.propTypes }

export default MaterialSipSharp

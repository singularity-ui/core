import React from 'react'

import Icon from '../Icon'

const MaterialLunchDiningSharpSvg = props => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    enable-background="new 0 0 24 24"
    height="24"
    viewBox="0 0 24 24"
    width="24"
    {...props}
  >
    <g>
      <rect fill="none" height="24" width="24" />
    </g>
    <g>
      <g>
        <rect height="5" width="20" x="2" y="16" />
        <path d="M18.66,11.5c-1.95,0-2.09,1-3.33,1c-1.19,0-1.42-1-3.33-1c-1.95,0-2.09,1-3.33,1c-1.19,0-1.42-1-3.33-1 c-1.95,0-2.09,1-3.33,1v2c1.9,0,2.17-1,3.35-1c1.19,0,1.42,1,3.33,1c1.95,0,2.09-1,3.33-1c1.19,0,1.42,1,3.33,1 c1.95,0,2.09-1,3.33-1c1.19,0,1.4,0.98,3.32,1l-0.01-1.98C20.38,12.19,20.37,11.5,18.66,11.5z" />
        <path d="M22,9c0.02-4-4.28-6-10-6C6.29,3,2,5,2,9v1h20L22,9L22,9z" />
      </g>
    </g>
  </svg>
)

const MaterialLunchDiningSharp = props => <Icon as={MaterialLunchDiningSharpSvg} {...props} />

MaterialLunchDiningSharp.defaultProps = { ...Icon.defaultProps }

MaterialLunchDiningSharp.propTypes = { ...Icon.propTypes }

export default MaterialLunchDiningSharp

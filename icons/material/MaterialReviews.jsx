import React from 'react'

import Icon from '../Icon'

const MaterialReviewsSvg = props => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    enable-background="new 0 0 24 24"
    height="24"
    viewBox="0 0 24 24"
    width="24"
    {...props}
  >
    <g>
      <path d="M0,0h24v24H0V0z" fill="none" />
    </g>
    <g>
      <path d="M20,2H4C2.9,2,2,2.9,2,4v18l4-4h14c1.1,0,2-0.9,2-2V4C22,2.9,21.1,2,20,2z M13.57,11.57L12,15l-1.57-3.43L7,10l3.43-1.57 L12,5l1.57,3.43L17,10L13.57,11.57z" />
    </g>
  </svg>
)

const MaterialReviews = props => <Icon as={MaterialReviewsSvg} {...props} />

MaterialReviews.defaultProps = { ...Icon.defaultProps }

MaterialReviews.propTypes = { ...Icon.propTypes }

export default MaterialReviews
